...
async function loadEvents() {
    try {
        ...
        if (res.ok) {
            ...
            events = data.events || [];
            const selector = document.getElementById("pv-meet-selector") as HTMLSelectElement;
            selector.innerHTML = "";
            events.forEach(e => {
            ...
            selector.addEventListener("change", (e) => {
                currentEventCode = (e.target as HTMLSelectElement).value;
                loadEventData();
            });
        }
    } catch (e) {
...
async function fetchSchedule() {
    ...
    if (res.ok) schedule = (await res.json()).schedule || [];
}
...
async function fetchRankings() {
    ...
    if (res.ok) rankings = (await res.json()).rankings || [];
}
...
function renderDashboard() {
    ...
    for (const m of sortedSchedule) {
        ...
        const isOurMatch = m.teams.some((t: any) => t.teamNumber === loggedInTeamId);
        if (isOurMatch && mTime > now) {
            upcomingMatches.push(m);
            if (!nextMatchForTeam) nextMatchForTeam = m;
        }
    }
    ...
    upcomingMatches.slice(0, 3).forEach(m => {
        ...
        const redTeams = m.teams.filter((t: any) => t.station.startsWith('Red')).map((t: any) => `<span class="${t.teamNumber === loggedInTeamId ? 'pv-my-team' : ''}">${t.teamNumber}</span>`).join(', ');
        const blueTeams = m.teams.filter((t: any) => t.station.startsWith('Blue')).map((t: any) => `<span class="${t.teamNumber === loggedInTeamId ? 'pv-my-team' : ''}">${t.teamNumber}</span>`).join(', ');
...
function loadMatchTeamInfo(match: any) {
