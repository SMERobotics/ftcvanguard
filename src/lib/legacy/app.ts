...
let scheduleOffsetEventCode: string = "";
...
function showView(viewName: string): void {
    const viewId = getViewIdFromName(viewName);
...
function normalizeEventCode(eventCode: string | null | undefined): string {
...
async function loadScheduleOffset(rerenderIfChanged: boolean = false, eventCode: string = currentEventCode) {
    ...
    try {
        ...
        if (normalizeEventCode(currentEventCode) !== normalizedEventCode) return;
...
async function saveScheduleOffset(offsetMinutes: number, eventCode: string = currentEventCode): Promise<boolean> {
    const token = localStorage.getItem("token");
    if (!token) return false;

    const normalizedEventCode = normalizeEventCode(eventCode);
    if (!normalizedEventCode) return false;
    ...
    try {
        ...
        if (normalizeEventCode(currentEventCode) !== normalizedEventCode) return false;
        const data = await response.json() as ScheduleOffsetResponse;
...
function attachUsersSearchListeners(): void {
...
function attachRegistrationsListeners(): void {
...
function attachNotesSearchListeners(): void {
...
async function loadEvents() {
    ...
    try {
        ...
        const data = await response.json();
        const events: EventInfo[] = data.events || [];
...
async function populateMeetSelectorWithStateRestore(events: EventInfo[], urlState: AppState) {
    ...
    let defaultEvent: EventInfo | undefined;
...
function populateMeetSelector(events: EventInfo[]) {
    const selector = document.getElementById("meet-selector") as HTMLSelectElement;
    if (!selector) return;
...
async function initializeLegacyApp(): Promise<void> {
    if (IS_CAPACITOR) {
...
export async function bootLegacyApp(): Promise<void> {
    if (legacyAppInitialized) return;
